...
class Money:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.amount, Decimal):
            raise InvalidMoneyAmountError("Money amount must be a Decimal.")

        if not isinstance(self.currency, Currency):
            raise TypeError("Money currency must be an instance of Currency.")
    ...
    def from_value(
        ...
        try:
            ...
            raise InvalidMoneyAmountError(f"Invalid monetary amount: {amount!r}") from e

        if not decimal_amount.is_finite():
            raise InvalidMoneyAmountError("Monetary amount must be finite")
    ...
    def multiply(self, factor: Decimal | int | str) -> Money:
        ...
        if not decimal_factor.is_finite():
            raise InvalidMoneyAmountError("Multiplication factor must be finite")
    ...
    def divide(self, divisor: Decimal | int | str) -> Money:
        ...
        if not decimal_divisor.is_finite():
            raise InvalidMoneyAmountError("Division divisor must be finite")

        if decimal_divisor == 0:
            raise InvalidMoneyAmountError("Division by zero is not allowed")

        return Money(amount=self.amount / decimal_divisor, currency=self.currency)
